/*
l * Copyright (C) 2012 Sistemas Operativos - UTN FRBA. All rights reserved.
 *
...
t_metadata_program* metadata_desde_literal(const char* literal){
	...
	while( position<tamanioTotal ){
		...
		t_instructions auxinst = { .start = position, .offset = strlen(buffer) +1 };
		...
		if( string_starts_with(buffer, TEXT_BEGIN) ){
			...
			if(	string_starts_with(buffer, TEXT_START_LABEL) ){
				...
				ret->cantidad_de_funciones++;
					...
					ret->instrucciones_serializado = realloc( ret->instrucciones_serializado, sizeof(t_instructions) * ret->instrucciones_size );
					memcpy( ret->instrucciones_serializado + ret->instrucciones_size - 1, &auxinst, sizeof(t_instructions) );	//La pone en el programa
			}
		}
